...
public class VideoValidator extends Validator {
    ...
    private final int DESCRIPTION_MAX_LENGTH = 4000;
    ...
    private void checkTitleConstraints() {
        ...
        if (title == null) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'title' should not be null"));
            return;
        }

        if (title.isBlank()) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'title' should not be empty"));
            return;
        }
    ...
    private void checkDescriptionConstraints() {
        ...
        if (description == null) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'description' should not be null"));
            return;
        }

        if (description.isBlank()) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'description' should not be empty"));
            return;
        }
    ...
    private void checkLaunchedAtConstraints() {
        if (this.video.getLaunchedAt() == null) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'launchedAt' should not be null"));
        }
    }

    private void checkRatingConstraints() {
        if (this.video.getRating() == null) {
            this.validationHandler().append(new com.codeflix.admin.catalog.domain.validation.Error("'rating' should not be null"));
        }
    }
}
